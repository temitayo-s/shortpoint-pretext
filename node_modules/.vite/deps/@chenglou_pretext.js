...
function findChunkIndexForStart(prepared, segmentIndex) {
	for (let i = 0; i < prepared.chunks.length; i++) if (segmentIndex < prepared.chunks[i].consumedEndSegmentIndex) return i;
	return -1;
}
...
function layoutNextLineRange(prepared, start, maxWidth) {
	...
	if (prepared.simpleLineWalkFastPath) return layoutNextLineRangeSimple(prepared, normalizedStart, maxWidth);
	...
	for (let i = normalizedStart.segmentIndex; i < chunk.endSegmentIndex; i++) {
		...
		if (lineW + w > maxWidth + lineFitEpsilon) {
			...
			if (pendingBreakSegmentIndex >= 0 && pendingBreakFitWidth <= maxWidth + lineFitEpsilon) return finishLine(pendingBreakSegmentIndex, 0, pendingBreakPaintWidth);
			if (w > maxWidth && breakableWidths[i] !== null) {
...
function layoutNextLineRangeSimple(prepared, normalizedStart, maxWidth) {
	...
	for (let i = normalizedStart.segmentIndex; i < widths.length; i++) {
		...
		if (lineW + w > maxWidth + lineFitEpsilon) {
			...
			if (pendingBreakSegmentIndex >= 0) return finishLine(pendingBreakSegmentIndex, 0, pendingBreakPaintWidth);
			if (w > maxWidth && breakableWidths[i] !== null) {
				const currentLine = finishLine();
				if (currentLine !== null) return currentLine;
				const line = appendBreakableSegmentFrom(i, 0);
